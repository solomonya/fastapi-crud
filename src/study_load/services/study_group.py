...
from ..models import Sgroup, StudyGroupBase, Speciality
from ..models.study_group import SgroupRead
...
class StudyGroupService:
    ...
    def get_one(self, id: int) -> Sgroup:
        return self._get(id)

    def get_all(self) -> List[Tuple[Sgroup, Speciality]]:
        groups = self.session.exec(
            select(Sgroup, Speciality).join(Speciality)).all()
        return groups
